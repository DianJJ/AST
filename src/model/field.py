...
class Field(nn.Module):
    ...
    def forward(self, x, latent=None):
        #x[642,3] latent[32, 642, 128]
        if latent is not None:
            N, B = x.shape[-2], len(latent)

            x = x[None] if len(x.shape) == 2 else x
            latent = latent[:, None] if len(latent.shape) == 2 else latent
            x = self.act((self.linear_x(x) + self.linear_z(latent)).view(B * N, -1))
            # the output of linear_x(x) and linear_z is 128 dimension
...
class ProgressiveField(nn.Module):
    ...
    def forward(self, x, features):
        if len(features.shape) == 2:
            ...
            B, N, C, device = features.size(0), features.size(1), self.latent_size, x.device
            latent_final = self.regressor(features)
            if self.act_idx < self.n_powers:
                p = self.current_code_size
                mask = torch.zeros(B, N, C, device=device)
