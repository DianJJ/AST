...
class Trainer:
    ...
    def run(self):
        ...
        for epoch in range(self.epoch_start, self.n_epoches + 1):
            ...
            for batch, (images, _labels) in enumerate(self.train_loader, start=1):
                ...
                #print('time of a run_single_batch_train', time.time() - start_time)
                start_time = time.time()
                if cur_iter % self.train_stat_interval == 0 and self.is_master:
                    self.log_train_metrics(cur_iter, epoch, batch)
                #print('time of a log_train_metrics', time.time() - start_time)
                if cur_iter % self.val_stat_interval == 0 and self.is_master:
                    if len(self.val_loader.dataset) > 10:
                        start_time = time.time()
                        scores=self.run_val_and_log(cur_iter, epoch, batch)
                        #print(scores.keys())
                        if 'chamfer-L1-ICP' in scores.keys():
                            if scores['chamfer-L1-ICP'] < chamfer_best_score:
                                chamfer_best_score = scores['chamfer-L1-ICP']
                                self.save_best(epoch=epoch, batch=batch, checkpoint=True)
                                print("best val score is :", chamfer_best_score)
                        if 'mask_iou' in scores.keys():
                            if scores['mask_iou'] > mask_best_score:
                                mask_best_score = scores['mask_iou']
                                self.save_best(epoch=epoch, batch=batch, checkpoint=True)
                                print("best val score is :", mask_best_score)
                        print('time of a run_val_and_log', time.time() - start_time)
                    start_time = time.time()
                    self.log_visualizations(cur_iter)
                    print('time of a log visualization', time.time() - start_time)
                    self.save(epoch=epoch, batch=batch)
                cur_iter += 1
                #print('time of a iteration', time.time()-start_time_iteration)
            print('time of a epoch', time.time()-start_time_epoch)
    ...
    def save_best(self, epoch, batch, checkpoint=False):
        state = {
            "epoch": epoch, "batch": batch, "model_name": self.model.name, "model_kwargs": self.model.init_kwargs,
            "model_state": self.model.state_dict(), "optimizer_state": self.optimizer.state_dict(),
            "scheduler_state": self.scheduler.state_dict(),
        }
        name = f'model_best.pkl' if checkpoint else 'model.pkl'
        torch.save(state, self.run_dir / name)
        print_log(f"Model saved at {self.run_dir / name}")
...
if __name__ == '__main__':
    ...
    n_gpus, n_nodes = cfg['training'].get('n_gpus', 2), cfg['training'].get('n_nodes', 1)
    ...
    if n_gpus > 1:
        ...
        print('time of a trainer',time.time() - start_time)
